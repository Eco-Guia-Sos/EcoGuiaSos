...
/**
 * Configuración de Regiones Geoestadísticas de México
 */
...
export const REGIONS_MX: Record<string, Region> = {
  norte: {
    name: 'Norte',
    states: ['02', '03', '05', '08', '19', '25', '26', '28'],
    color: '#4ecdc4'
  },
  bajio: {
    name: 'Bajío / Occidente',
    states: ['01', '06', '10', '11', '14', '16', '18', '24', '32'],
    color: '#ff6b6b'
  },
  centro: {
    name: 'Centro',
    states: ['09', '13', '15', '17', '21', '22', '29'],
    color: '#ffe66d'
  },
  sur: {
    name: 'Sur / Sureste',
...
export function getRegionByState(stateCode: string) {
...
const ESTADOS_MX_FALLBACK: Array<{ code: string; name: string; centroid: { coordinates: [number, number] } }> = [
...
const MUNICIPIOS_FALLBACK: Record<string, Array<{ code: string; name: string; centroid?: { coordinates: [number, number] } }>> = {
  '09': [
...
export const TerritoryService = {
  _cache: {
    states: null as Territory[] | null,
    layers: null as any[] | null,
    geometries: new Map<string, any>(),
    municipalities: new Map<string, Territory[]>()
  },
  ...
  async getStates(): Promise<Territory[]> {
    ...
    try {
      ...
      console.warn('[TerritoryService] Error al obtener estados de Supabase:', err.message)
    }
  ...
  async getMunicipalities(stateCode: string): Promise<Territory[]> {
    ...
    if (fallbackData) {
      const result = fallbackData.map(m => ({
        ...
        centroid: m.centroid ? { coordinates: m.centroid.coordinates } as any : null,
        _isFallback: true
      }))
      this._cache.municipalities.set(stateCode, result)
      return result
  ...
  async detectStateFromCoords(lat: number, lng: number): Promise<Territory | null> {
  ...
  async getGeometry(territoryId: string): Promise<any> {
  ...
  async getStatesByRegion(regionId: string) {
    const allStates = await this.getStates()
    return allStates.filter(s => s.region && s.region.id === regionId)
  },

  async fetchEventsByTerritory(territoryId: string): Promise<any[]> {
    const { data, error } = await supabase
      .rpc('get_events_in_territory', { p_territory_id: territoryId })
      
    if (error) {
      console.warn('[TerritoryService] Fallo filtro espacial RPC:', error.message)
    }
  ...
  async fetchEventsByRegion(regionId: string): Promise<any[]> {
    const states = await this.getStatesByRegion(regionId)
    const stateIds = states.map(s => s.id).filter(id => id !== null) as string[]
